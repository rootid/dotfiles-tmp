...
function go_schedule_my_tom() {
  ...
  _go_schedule_my_day $tom_dd_mm
}

function go_sch_interview() {
  # go_schedule_interview "company:name" "$time"
  # eg. go_sch_interview "gman" "08/21 12:00PM"
  who_=$1
  when_=$2
  gcalcli add --title "${who_}" --where "somewhere" --when "'${when_}'" --duration 15 --description 'Yo!' --reminder 2
}
